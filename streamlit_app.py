...
.stApp::before {
    ...
    background: rgba(149,100,255,0.35); border-radius: 50%;
    filter: blur(180px); z-index: 0; pointer-events: none;
}
.stApp::after {
    content: ''; position: fixed;
    width: 580px; height: 580px; top: 10%; right: -120px;
    background: rgba(56,189,248,0.28); border-radius: 50%;
    filter: blur(160px); z-index: 0; pointer-events: none;
}

#MainMenu, footer, header { visibility: hidden; }
.block-container { padding: 2.5rem 3rem 3rem 3rem !important; max-width: 1400px !important; position: relative; z-index: 1; }

/* Glass on all st containers */
...
.hero-title {
    ...
    line-height: 1.05; color: #0f172a; margin-bottom: 0.5rem;
}
.hero-title span {
    background: linear-gradient(135deg, #7c3aed, #6366f1, #0ea5e9);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent;
}
.hero-sub { font-size: 0.95rem; color: #64748b; line-height: 1.6; max-width: 560px; margin-bottom: 1.5rem; }
...
div[data-testid="stToggle"] p {
    ...
    color: #7c3aed !important; margin: 0 !important;
}
div[data-testid="stToggle"] span[data-testid="stToggleSwitch"] {
    background-color: #7c3aed !important;
}
/* the actual toggle track */
div[data-testid="stToggle"] input:checked + div,
div[data-testid="stToggle"] input:checked ~ div[data-testid="stToggleSwitch"] {
    background-color: #7c3aed !important;
}
</style>
...
with st.container(border=True):
    ...
    <div class="hero-sub">
        100 designer fragrances mapped by olfactory similarity — an AI model analyzes each scent's
        notes and accords, then projects them into 2D space. Similar fragrances cluster together.
    </div>
    <div class="stat-row">
        <div class="stat-pill"><span class="dot" style="background:#7c3aed"></span>100 Fragrances</div>
        <div class="stat-pill"><span class="dot" style="background:#ec4899"></span>{df['Accords'].nunique()} Accord Groups</div>
        <div class="stat-pill"><span class="dot" style="background:#10b981"></span>{(variance_ratio[0]+variance_ratio[1])*100:.0f}% Variance explained</div>
    </div>
    """, unsafe_allow_html=True)
